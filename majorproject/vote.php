...
// Fetch group name
$get_group_name_query = mysqli_query($connect, "SELECT `name` FROM `users` WHERE `id`='$gid'");
$group_name_row = mysqli_fetch_assoc($get_group_name_query);
...
if ($update_votes && $update_user_status) {
    ...
    // Alert with group name
    echo '
    <script>
    alert("Successfully voted for ' . $group_name . '");
    window.location="dashboard.php";
    </script>';
} else {
    echo '
    <script>
    alert("Failed to vote for ' . $group_name . '");
    window.location="dashboard.php";
    </script>';
}
?>
